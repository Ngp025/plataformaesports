...
// SCSS imports
import '../../../styles/customers/ciudadesports.scss';
// Logim
...
class CiudadEsports extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  openLogin(validating) {
    if (validating) {
      CiudadEsports.prototype.loginState('validation');
    }
    if (event.target) {
      gameClicked.pop();
      gameIDTs.pop();
  ...
  async locallogin() {
    ...
    //Form Vars
    if (emailL && passwordL) {
      CiudadEsports.prototype.loginState('updating');
      //CiudadEsports.prototype.loginState('updating');
      await fetch(`users/betalogin/${emailL}/${passwordL}`)
        .then((res) => res.json())
        .then((json) => {
          if (json.alert) {
            CiudadEsports.prototype.loginState('login', true);
            alert(json.alert);
          } else {
            //CiudadEsports.prototype.loginState('congrats');
  ...
  localRegister() {
    ...
    function localLoginReload(json, send) {
      ...
      sessionStorage.setItem('userSession', json.send);
      localStorage.setItem('userData', JSON.stringify(json.send));
      send ? CiudadEsports.prototype.loginState('validation') : location.reload();
    }
    async function sendMessage(json) {
      ...
      await fetch(`api/games/sendMessage/${messsageType}/${email}`)
        ...
        .then((jsonMess) => {
          ...
          if (jsonMess.alert) {
            CiudadEsports.prototype.loginState('register', true);
  ...
  async gameModalHandler() {
    ...
    var CiudadEsportsContent = document.getElementById(
      'CiudadEsports-box'
    );
    ...
    closeModal.addEventListener('click', ()=>{
      CiudadEsportsContent.style.opacity = 1;
      mainLogo.style.opacity = 1;
    })
    if (instance.isOpen === false) {
      //console.log('AQUI')
      instance.open();
      CiudadEsportsContent.style.opacity = 0.2;
    ...
    document.addEventListener('keydown', (key) => {
      if (key.key === 'Escape') {
        instance.isOpen === false
          ? (CiudadEsportsContent.style.opacity = 1, mainLogo.style.opacity = 1)
          : '';
      }
    });
  }
  ...
  componentDidMount() {
    ...
    document.addEventListener('keydown', () => {
      if (event.code === 'Escape') {
        ...
        var modalOverlay = document.getElementsByClassName('modal-overlay');
        //console.log(CiudadEsportsContent)
        if (CiudadEsportsContent.style.display === 'none') {
          this.gameModalHandler();
          CiudadEsportsContent.style.display = 'block';
          //console.log(CiudadEsportsContent)
        }
      }
    });
  ...
  render() {
    return (
      <div id='responsive-CiudadEsports'>
        <div
          id='CiudadEsports-wrapper-image'
          className='CiudadEsports-wrapper-image'></div>
        <div
          id='CiudadEsports-wrapper-gradient'
          className='CiudadEsports-wrapper-gradient'></div>
        <div
          id='plataformaesports-CiudadEsports'
          className='CiudadEsports-wrapper'>
          <div
            id='CiudadEsports-content'
            className='CiudadEsports-content'>
            ...
            {gamesModal(
              ...
              CiudadEsports.prototype.openLogin
            )}
            ...
            <div id='CiudadEsports-box'>
              <h4
                id='CiudadEsports-legend'
                className='CiudadEsports-legend'>
              ...
              <div id='sponsor-footer-1' className='sponsor-footer-prod'>
                <img
                  ...
                  src='https://res.cloudinary.com/versus/image/upload/v1597775514/Statics_images/Customers%20Sponsors/Rosario/attdaju0qjtcue38vbxt.png'></img>
              </div>
            ...
            <div
              ...
              <div
                ...
                {login.loginBeta(
                  CiudadEsports.prototype.loginState,
                  this.locallogin,
                  this.state.personalize !== 'load'
                ...
                {login.validationModal(
                  CiudadEsports.prototype.localValidation,
                  this.state.personalize !== 'load'
                    ? this.state.personalize.mainColor
                    : 0
                )}
                ...
                <i
                  ...
                  onClick={() =>
                    CiudadEsports.prototype.loginState('login')
                  }>
                  arrow_back
                </i>
              </div>
