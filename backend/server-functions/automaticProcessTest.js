...
const { checkDiff } = require('./clockFunctions.js');
...
async function checkWhoPass(actualBracketInfo, preparedInfo, lifes) {
  ...
  function checkCementery(lifes, player0, player1, numbersOfBattles) {
    console.log(player0, player1, 'checkCementery');
    var deathOf0 = 0;
    var deathOf1 = 0;

    for (var key in lifes) {
      if (key === player0.ID) {
        deathOf0 = lifes[key];
      }
      if (key === player1.ID) {
    ...
    if (deathOf0 < deathOf1) {
      informationReturn.usersWhoPassToTiebreak.push(player0);
      informationReturn.usersWhoPassToNextRound.push(player1);
      informationReturn.whichBattlesExistAndPass.push(numbersOfBattles);
    } else if (deathOf0 > deathOf1) {
      informationReturn.usersWhoPassToTiebreak.push(player1);
      informationReturn.usersWhoPassToNextRound.push(player0);
      informationReturn.whichBattlesExistAndPass.push(numbersOfBattles);
    } else {
      specialCase(player0, player1, numbersOfBattles);
    }
  }
    ...
    if (actualBracketInfo[numbersOfBattles][3].length === 1) {
      ...
      if (actualBracketInfo[numbersOfBattles][3].length < 1) {
        ...
        if (actualBracketInfo[numbersOfBattles][2].length > 1) {
          ...
          if (lifes > 1) {
            checkCementery(
              actualBracketInfo[numbersOfBattles][4].lifes,
              user0Info,
              user1Info,
              numbersOfBattles
            );
...
async function configureTimerPerBattle(verify, game, checkOrg, actualMoment) {
  ...
  var UTCarg = 3;
  var newSt = await toChange.add(totalTime, 'm').subtract(UTCarg, 'h').toDate();
  //var returTthis = newSt.split(' ')
  //console.log(totalTime, "totalTime")
  return newSt; //`${returTthis[0]}T${returTthis[1]}:00`;
}

async function configureTimer(timer, actualMoment, edit, lifes) {
  var multiplier = 0;
  lifes === 1 ? (multiplier = lifes) : (multiplier = lifes + 1);

  var multy = timer.timerPerGame * multiplier;
  ...
  edit['lastChangeGame'] = await configureTimerPerBattle(
    timer.timerPerVerify,
    timer.timerPerGame,
  ...
  edit['lastChangeDate'] = await configureTimerPerBattle(
    0, //timer.timerPerVerify,
    1380, //multy,
    0, //timer.timerForCheckImages,
    actualMoment
  );
  console.log(edit);
  return edit;
...
async function passRound(data, actualValuesT, objTime, actOrganizer, server) {
  ...
  async function saveForANextBracket(
    ...
    timer = await configureTimer(data.timer, timeActualMoment, timer, lifes);
  ...
  if (gMaxActualValue.alert) {
    ...
    var infoWhoPass = await checkWhoPass(
      actualBracketInfo,
      preparedInfo,
      lifes
    );
...
function countDown_Per_Bracket(timer, actualMoment) {
  ...
  var checkingLastDateClone = moment(lastChangeDate, 'LLLL').clone();
  var checkingLastDate = checkingLastDateClone.add(3, 'hours');
  var differenceInHoursBettween = checkingLastDate.diff(actualMoment, 'hours');
  var differenceInMinsBettween = checkingLastDate.diff(actualMoment, 'mins');
...
async function actualIntanceIdentify(IDT) {
  var json = {
    option1: [],
    option2: [],
    option3: [],
  ...
  json.option1 = await Tournament.findById(
    { _id: IDT },
    {
      initial: {
  ...
  json.option2 = await Tournament.findById(
    { _id: IDT },
    {
      mid: {
        $elemMatch: { active: true },
  ...
  json.option3 = await Tournament.findById(
    { _id: IDT },
    {
      final: {
        $elemMatch: { active: true },
  ...
  console.log(json);
  if (json.initial !== []) {
    return json.option1.initial;
  }
  if (json.mid !== []) {
    return json.option2.mid;
  }
  if (json.final !== []) {
    return json.option3.final;
  }
}
    ...
    async (req, arrAllTourn) => {
      ...
      for (var i = 0; i < arrAllTourn.length; i++) {
        ...
        if (arrAllTourn[i].iTournament === 3) {
          ...
          console.log(countDownPerBracket);
...
async function changeTimePerInstance(server) {
  console.log('changeTimePerInstance');
  var change = [];
  var options = [];
  var pass = false;

  async function changeIntance(obj) {
    var diference = checkDiff(new Date(obj.timer.lastChangeGame));
    console.log(diference, 'diference');
    if (diference[0] === true) {
      var actualMoment = whatTimeIsIt();
      console.log(obj.timer.lastChangeGame, 'ANtES');
      obj.timer['lastChangeGame'] = await configureTimerPerBattle(
        0,
        obj.timer.timerPerGame,
        0,
        actualMoment
      );
      console.log(obj.timer.lastChangeGame, 'DESPUES');
      change.push(obj);
      pass = true;
    }
  }

  async function modifyValues(tID, timer) {
    var va = await Tournament.findByIdAndUpdate(
      { _id: tID },
      {
        timer: timer,
      }
  ...
  options = await Tournament.find(
    {
      validTournament: true,
      isFinished: false,
      iTournament: 3,
      customer: 'CIUDAD',
      game: 'pes20', //<----------------- FOR TESTTING
    },
    { timer: 1 }
  );

  for (var i = 0; i < options.length; options++) {
    var pass = false;
    if (options[i]) {
      await changeIntance(options[i]);
      if (pass === true) {
        await modifyValues(options[i]._id, options[i].timer);
        pass = false;
      }
    }
  }
